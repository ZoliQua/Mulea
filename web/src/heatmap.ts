...
export interface HeatmapLayout {
  width: number; height: number; labelW: number; cellW: number; cellH: number; colLabelH: number;
...
export function heatmapLayout(result: AnalysisResult, opts: HeatmapOptions): HeatmapLayout {
  ...
  const rows: HeatRow[] = sig.map((r, i) => ({ id: r.ontology_id, label: r.ontology_name, y: colLabelH + i * opts.cellH, score: rowScore(r) }));
  ...
  return {
    ...
    height: colLabelH + sig.length * opts.cellH,
    labelW, cellW: opts.cellW, cellH: opts.cellH, colLabelH, rows, cols, cells,
  };
}
